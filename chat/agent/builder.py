...
_ORIGINAL_DEEPSEEK_GET_REQUEST_PAYLOAD = cast(
...
def _get_request_payload_with_reasoning_content(
    ...
    **kwargs: Any,
    ...
    payload = _ORIGINAL_DEEPSEEK_GET_REQUEST_PAYLOAD(self, input_, stop=stop, **kwargs)
    ...
    for source_message, payload_message in zip(
        ...
        if not (
            ...
            continue
        if REASONING_CONTENT_KEY not in source_message.additional_kwargs:
            continue
        reasoning_content = source_message.additional_kwargs[REASONING_CONTENT_KEY]
        if reasoning_content is not None:
            payload_message[REASONING_CONTENT_KEY] = reasoning_content
...
def _chain_model_call_handlers(
    ...
    def _to_composed_result(
        ...
        return _ComposedExtendedModelResponse(
            model_response=model_response, commands=commands
        )
...
def _chain_async_model_call_handlers(
    ...
    def compose_two(
        ...
        inner: _AsyncModelCallHandler[ContextT]
        | _ComposedAsyncModelCallHandler[ContextT],
    ) -> _ComposedAsyncModelCallHandler[ContextT]:
...
def _handle_structured_output_error(
    ...
    if isinstance(handle_errors, type):
        if issubclass(handle_errors, Exception) and isinstance(
...
def create_agent(  # noqa: PLR0915
    ...
    *,
    ...
    built_in_tools: list[dict[str, Any]] = [
        cast("dict[str, Any]", t) for t in tools if isinstance(t, dict)
    ]
    ...
    wrap_model_call_handler: _ComposedModelCallHandler[ContextT] | None = None
    if middleware_w_wrap_model_call:
        sync_handlers: list[_ModelCallHandler[ContextT]] = [
            cast(
                "_ModelCallHandler[ContextT]",
                traceable(
                    name=f"{m.name}.wrap_model_call", process_inputs=_scrub_inputs
                )(m.wrap_model_call),
            )
    ...
    awrap_model_call_handler: _ComposedAsyncModelCallHandler[ContextT] | None = None
    if middleware_w_awrap_model_call:
        async_handlers: list[_AsyncModelCallHandler[ContextT]] = [
            cast(
                "_AsyncModelCallHandler[ContextT]",
                traceable(
                    name=f"{m.name}.awrap_model_call", process_inputs=_scrub_inputs
                )(m.awrap_model_call),
            )
