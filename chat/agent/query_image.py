...
class QueryImageInput(BaseModel):
    ...
    time: str = Field(
        description="要问询的包含图片的目标用户消息的具体发送时间，格式为“YY-mm-dd HH:MM:SS”。"
    )
    query: str = Field(
...
def query_image(time: str, query: str, runtime: ToolRuntime) -> str:
    ...
    for msg in _runtime.state["full_messages"]:
        if (
            ...
            if len(time.strip()) > MIN_FOCUS_LENGTH and time_text in time.strip():
                ...
                break
    if not message:
        return "没有找到对应的消息。请检查时间格式是否正确。"
    if not message.images:
        return "指定的时间指向的消息没有图片。"
