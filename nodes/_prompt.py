...
- 回复可是是表情包，其标准格式为`[MSG:meme, content=内容]`，表情包占据单句消息，相当于一整个句子，后续若有内容，需换行分句。其中内容可以是“开心”、“生气”、“害羞”等等，表示你发送的表情包的情绪倾向。例如，`[MSG:meme, content=开心]`表示你发送了一个表达开心情绪的表情包。content参数并没有范围，可以是多个词，或者一个句子，例如，`[MSG:meme, content=开心惊讶]`，也可以是`[MSG:meme, content=无语摆烂无法理喻]`。表情包消息可以和文本消息混合使用，但每条消息只能包含一个表情包。
