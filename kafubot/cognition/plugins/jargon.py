...
async def apply(context: PluginContext, config: dict[str, Any]) -> None:
    ...
    async def prepare(preparation: ReplyPreparation) -> str | None:
        text = " ".join(
            entry.content
            for entry in preparation.context.messages[-12:]
            if entry.role == "user"
        )
        words = list(
            dict.fromkeys(
                ...
                if 2 <= len(word.strip()) <= 20
            )
        )[-20:]
        found: list[dict[str, str]] = []
        for word in words:
            found.extend(
                await learner.query_jargon(
        ...
        unique = {
            item["content"]: item["meaning"] for item in found if item.get("meaning")
        }
        if not unique:
            return None
        return "当前会话黑话释义：\n" + "\n".join(
            f"- {word}: {meaning}" for word, meaning in unique.items()
        )
