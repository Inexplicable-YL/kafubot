...
class ReplyEffectMiddleware(AgentMiddleware[ManagerState, ManagerContext]):
    ...
    async def observe_messages(
        self,
        session_id: str,
        incoming: Sequence[UserMessage],
    ) -> None:
        """Observe follow-ups directly from the unified plugin event."""
    ...
    async def record_reply(
        ...
        *,
        text: str,
        action: dict[str, Any],
        selected_behavior_ids: Sequence[int] = (),
        selected_expression_ids: Sequence[int] = (),
        sent_at: datetime | None = None,
    ) -> None:
        """Start an observable-effect window for a committed native reply."""
        self._pending.setdefault(session_id, []).append(
            PendingReply(
                effect_id=str(uuid.uuid4()),
                session_id=session_id,
                sent_at=self._as_utc(sent_at or datetime.now(UTC)),
                text=text,
                action=dict(action),
    ...
    async def apply_reply_effect(
        ...
        async with anyio.create_task_group() as task_group:
            if behavior is not None:
                ...
                async def apply_behavior() -> None:
                    await active_behavior.apply_observable_effect(
                ...
                task_group.start_soon(apply_behavior)
            if expression is not None:
                active_expression = expression

                async def apply_expression() -> None:
                    await active_expression.apply_observable_effect(
                        expression_ids=pending.selected_expression_ids,
                        metrics=metrics,
                        confidence=confidence,
                    )

                task_group.start_soon(apply_expression)

    async def observe(event: ObservationEvent) -> None:
        await tracker.observe_messages(event.session_id, (event.message,))

    async def committed(event: ReplyCommitted) -> None:
        preparation = event.preparation
        await tracker.record_reply(
            preparation.context.session_id,
            text=event.result.full_text,
            action=preparation.contract.model_dump(mode="json"),
            selected_behavior_ids=list(
                preparation.metadata.get("selected_behavior_ids") or []
            ),
            selected_expression_ids=list(
                preparation.metadata.get("selected_expression_ids") or []
            ),
        )

    tracker.add_effect_observer(apply_reply_effect)
    context.resource("reply_effects", tracker)
    context.on_observe(observe)
    context.on_reply_committed(committed)
    context.clear_session(tracker.clear_session)
