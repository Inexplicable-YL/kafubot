...
class ConversationFrameMiddleware(AgentMiddleware[ManagerState, ManagerContext]):
    ...
    async def abefore_agent(
        ...
        events = [
            MessageEvent.from_user_message(session_id, message)
            for message in state.get("inputs", [])
        ]
        for event in events:
        ...
        latest_bot_message = next(
            (
                message.text
                for message in reversed(list(state.get("histories", [])))
        ...
        graph.bot_recently_spoke = any(
            ...
            for message in list(state.get("histories", []))[-10:]
        )
        frame = graph.frame()
    ...
    async def aafter_agent(
        ...
        if any(output["type"] in {"reply", "meme"} for output in state["outputs"]):
            self._graphs[runtime.context["session_id"]].observe_bot()
        return None
